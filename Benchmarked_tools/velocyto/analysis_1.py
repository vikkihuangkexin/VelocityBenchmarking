...
from velocyto.estimation import fit_slope, fit_slope_offset, fit_slope_weighted, fit_slope_weighted_offset
from velocyto.estimation import clusters_stats
...
class VelocytoLoom:
    ...
    def filter_cells(self, bool_array: np.ndarray) -> None:
        ...
        self.S, self.U = (X[:, bool_array] for X in (self.S, self.U))
    ...
    def knn_imputation(self, k: int=None, pca_space: float=True, metric: str="euclidean", diag: float=1,
        ...
        if balanced:
            ...
            bknn.fit(space.astype(np.float64))
