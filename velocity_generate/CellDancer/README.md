...
Behavior:
- With `--simulate` (default), preprocessing uses simulation defaults (map `X_dimred` to `X_umap`,
  set `top_gene` according to `n_vars`, use `scv.pp.filter_and_normalize(..., min_shared_counts=None, n_top_genes=top_gene)`,
