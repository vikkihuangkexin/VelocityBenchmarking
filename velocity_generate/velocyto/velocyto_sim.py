...
def add_velo(adata_New,adata_ori):
    ...
    if all(adata_ori.obs_names==adata_New.obs_names):
        velo_data = adata_ori.layers['ground_truth_velocity']
    else:
        # 获取 adataA 细胞在 adataB 中的索引位置
        cell_indices = [adata_ori.obs_names.get_loc(cell) for cell in list(adata_New.obs_names)]
...
def main(data_dir,data_file,save_dir):
    ...
    #         adata.obs_names_make_unique()
    #         # if ID == '48':
    #         #     target_cells = ['AGGGATGCATCACCCT', 'TTGCCGTGTCGAGTTT']
    #         #     adata = adata[~adata.obs_names.isin(target_cells), :]
    #         adata.write_loom(fr'/data_d/Velocity/data/simulation/loom/{data_file.split(".")[0]}.loom', write_obsm_varm=True)
    #     else:
    ...
    #             # if ID=='48':
    #             #     target_cells = ['AGGGATGCATCACCCT', 'TTGCCGTGTCGAGTTT']
    #             #     adata1 = adata1[~adata1.obs_names.isin(target_cells), :]
    #             adata1.write_loom(fr'/data_d/Velocity/data/simulation/loom/{data_file.split(".")[0]}.loom', write_obsm_varm=True)
    if not os.path.exists(fr'/data_d/Velocity/data/loom/{data_file.split(".")[0]}.loom'):
        adata.obsm['X_umap'] = adata.obsm['X_dimred']
        adata.write_loom(fr'/data_d/Velocity/D/simdata/loom/{data_file.split(".")[0]}.loom',
                                  write_obsm_varm=True)
    # del adata
    # Crate an analysis object
    if any(item.lower().find('ambiguous') != -1 for item in obsm_key):
        vlm = vcy.VelocytoLoom(fr'/data_d/Velocity/D/simdata/loom/{data_file.split(".")[0]}.loom')
    else:
        vlm = VelocytoLoom(fr'/data_d/Velocity/D/simdata/loom/{data_file.split(".")[0]}.loom')
    ...
    #
    # vlm.score_detection_levels(min_expr_counts=20, min_cells_express=30)
    # vlm.filter_genes(by_detection_levels=True)
    if vlm.S.shape[0] < 500:
        top_gene =200
    elif vlm.S.shape[0] < 3000:
        top_gene = (vlm.S.shape[0] // 500) * 500
        top_gene = min(top_gene, vlm.S.shape[0])
    else:
        top_gene = 3000
    # if ID.startswith('trifurcating'):
    #     max_expr_avg = 35000
    # else:
    #     max_expr_avg = 35000
    vlm.score_cv_vs_mean(top_gene, plot=True, max_expr_avg=350000)
    ...
    if vlm.ca['X_umap'].shape[0]<k*8:
        k = 4
        vlm.knn_imputation(n_pca_dims=n_comps, k=k, balanced=True, b_sight=k*8, b_maxl=k*4,n_jobs=32)
    else:
        vlm.knn_imputation(n_pca_dims=n_comps, k=k, balanced=True, b_sight=k * 8, b_maxl=k * 4,n_jobs=32)
    vlm.fit_gammas()
    vlm.predict_U()
    ...
    if 'cell' in ID and 'gene' in ID:
        data_out.obs_names = data_out.obs['obs_names']
        data_out.var_names = data_out.var['var_names']
        data_out = add_velo(data_out,adata)
...
if __name__ == '__main__':
    ...
    save_dir = '/data_d/Velocity/D/simdata/dis/velocyto'
    # save_dir = '/data_d/Velocity/data/simulation/simulation_result/zy_results/velocyto'
    # retrain = [52]
    # retrain.extend(list(range(47,len(datalist))))
    outlist=['disconnected_cell10000_gene10000','disconnected_cell1000_gene10000']
